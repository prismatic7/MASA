...
export function validateEventReceipt(value: unknown): ValidationResult<OperationReceipt> {
...
export const validateOperationReceipt = validateEventReceipt;
...
export const validatePublicRecord = auditPublicRecord;
...
function dedupeDiagnostics(diagnostics: readonly Diagnostic[]): Diagnostic[] {
  ...
  return diagnostics.filter((item) => {
    const key = [item.code, item.severity, item.instancePath, item.schemaPath ?? "", item.message].join(" ");
